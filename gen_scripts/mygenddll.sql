create or replace procedure mygenddl(p_type in varchar2, p_schema in varchar2, p_name in varchar2) as
    ...
    gc_not_null_as_alter            constant boolean := true;
    gc_print_private_synonyms       constant boolean := true;
    gc_print_public_synonyms        constant boolean := true;
    gc_strip_object_schema          constant boolean := true;
    gc_strip_tablespace_clause      constant boolean := true;
    gc_strip_segment_attrs          constant boolean := true;
    ...
    begin
        ...
        insert into sys.genddl_store (object_schema, object_type, object_name, ddl) values (p_schema, p_type, p_name, lower(replace(trim(p_clob),'"','')));
