...
create or replace procedure mygenddl(p_type in varchar2, p_schema in varchar2, p_name in varchar2) as
    ...
    gc_schema_name constant user_users.username      %type := p_schema;
    ...
    procedure print_nl;
    procedure print_clob (p_clob in clob, p_type in varchar2 default gc_object_type, p_name in varchar2 default gc_object_name);
    ...
    is
        ...
        l_ddl       clob;
    begin
        if substr(p_index_name,1,5) not in ('SYS_I','SYS_C') then
            /*
    ...
    begin
        ...
        dbms_lob.append(l_ddl, gc_sql_terminator);
        print_clob(ltrim(l_ddl),'SYNONYM',p_synonym_name);
        dbms_lob.freetemporary(l_ddl);
    ...
    is
        ...
        l_ddl       clob;
    begin
        if substr(p_constraint_name,1,5) not in ('SYS_I','SYS_C') then
            ...
            */
            ...
            print_clob(ltrim(rtrim(l_ddl, gc_blank_or_nl), gc_blank_or_nl) || gc_sql_terminator, p_object_type, p_constraint_name);
            dbms_lob.freetemporary(l_ddl);
        end if;
    ...
    begin
        ...
        insert into sys.genddl_store (object_schema, object_type, object_name, dependent_type, dependent_name, ddl) values (p_schema, gc_object_type, gc_object_name, p_type, p_name, lower(replace(trim(p_clob),'"','')));
