...
dependency_chains_out as (
    ...
    where
        owner = :B_OWNER
    --and type in ('SEQUENCE','TABLE','TYPE','TYPE BODY','FUNCTION','PROCEDURE','PACKAGE SPECIFICTION','PACKAGE BODY','PACKAGE','QUEUE','SYNONYM','VIEW','ROLE','DIRECTORY','TRIGGER','CONSTRAINTS')
    and name not like '%$%'
)
select
    *
...
dependency_chains (
    ...
    where
        a.owner = b.referenced_owner
        and b.referenced_owner not like '%SYS%'
        and a.name = b.referenced_name
        and a.type = b.referenced_type
)
cycle owner, name, type
...
dependency_chain_groups as (
    ...
    where
        owner = :B_OWNER
    --and type in ('SEQUENCE','TABLE','TYPE','FUNCTION','PROCEDURE','PACKAGE SPECIFICTION','PACKAGE BODY','PACKAGE','QUEUE','SYNONYM','VIEW','ROLE','DIRECTORY','TRIGGER','CONSTRAINTS')
    and name not like '%$%'
    group by
        owner, name, type
)
