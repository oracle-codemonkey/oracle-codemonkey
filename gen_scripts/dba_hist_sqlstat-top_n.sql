...
hist_dbtime as (
    ...
    where
        ...
        and b.end_interval_time <= to_date(:TO_TIME, 'YYYY-MM-DD HH24') + 1/24/4
...
hist_sqlstat2 as (
    select
        row_number() over (order by elapsed_time_delta_s desc) as rn,
...
from
    ...
    hist_dbtime c
where
    a.rn <= greatest(20, nvl(:MAX_ROWS, 50))
    and a.dbid = b.dbid (+)
...
hist_sqlstat2 as (
    select
        row_number() over (order by cpu_time_delta_s desc) as rn,
        case when sum(decode(command_name, 'PL/SQL EXECUTE', 0, 'CALL METHOD', 0, 
                    elapsed_time_delta_s)) over () > 0
            then round(100 * elapsed_time_delta_s
                    / sum(decode(command_name, 'PL/SQL EXECUTE', 0, 'CALL METHOD', 0,
        ...
        case when sum(decode(command_name, 'PL/SQL EXECUTE', 0, 'CALL METHOD', 0, 
                    cpu_time_delta_s)) over () > 0
            then round(100 * cpu_time_delta_s
                    / sum(decode(command_name, 'PL/SQL EXECUTE', 0, 'CALL METHOD', 0,
                            cpu_time_delta_s)) over (), 1)
            end as pct_cpu_time,
        case when sum(decode(command_name, 'PL/SQL EXECUTE', 0, 'CALL METHOD', 0,
                    cpu_time_delta_s)) over () > 0
            then round(100 * sum(decode(command_name, 'PL/SQL EXECUTE', 0, 'CALL METHOD', 0,
                                        cpu_time_delta_s))
                                    over (order by cpu_time_delta_s desc) 
                    / sum(decode(command_name, 'PL/SQL EXECUTE', 0, 'CALL METHOD', 0,
                            cpu_time_delta_s)) over (), 1)
            end as tot_pct_cpu_time,
        dbid,
        force_matching_signature,
        cnt_sql_id,
        samp_sql_id,
...
hist_sqlstat2 as (
    select
        row_number() over (order by iowait_delta_s desc) as rn,
        case when sum(decode(command_name, 'PL/SQL EXECUTE', 0, 'CALL METHOD', 0, 
                    elapsed_time_delta_s)) over () > 0
            then round(100 * elapsed_time_delta_s
                    / sum(decode(command_name, 'PL/SQL EXECUTE', 0, 'CALL METHOD', 0,
        ...
        case when sum(decode(command_name, 'PL/SQL EXECUTE', 0, 'CALL METHOD', 0, 
                    iowait_delta_s)) over () > 0
            then round(100 * iowait_delta_s
                    / sum(decode(command_name, 'PL/SQL EXECUTE', 0, 'CALL METHOD', 0,
                            iowait_delta_s)) over (), 1)
            end as pct_iowait,
        case when sum(decode(command_name, 'PL/SQL EXECUTE', 0, 'CALL METHOD', 0,
                    iowait_delta_s)) over () > 0
            then round(100 * sum(decode(command_name, 'PL/SQL EXECUTE', 0, 'CALL METHOD', 0,
                                        iowait_delta_s))
                                    over (order by iowait_delta_s desc) 
                    / sum(decode(command_name, 'PL/SQL EXECUTE', 0, 'CALL METHOD', 0,
                            iowait_delta_s)) over (), 1)
            end as tot_pct_iowait,
        dbid,
        force_matching_signature,
        cnt_sql_id,
        samp_sql_id,
...
hist_sqlstat2 as (
    select
        row_number() over (order by buffer_gets_delta desc) as rn,
        case when sum(decode(command_name, 'PL/SQL EXECUTE', 0, 'CALL METHOD', 0, 
                    elapsed_time_delta_s)) over () > 0
            then round(100 * elapsed_time_delta_s
                    / sum(decode(command_name, 'PL/SQL EXECUTE', 0, 'CALL METHOD', 0,
        ...
        case when sum(decode(command_name, 'PL/SQL EXECUTE', 0, 'CALL METHOD', 0, 
                    buffer_gets_delta)) over () > 0
            then round(100 * buffer_gets_delta
                    / sum(decode(command_name, 'PL/SQL EXECUTE', 0, 'CALL METHOD', 0,
                            buffer_gets_delta)) over (), 1)
            end as pct_gets,
        case when sum(decode(command_name, 'PL/SQL EXECUTE', 0, 'CALL METHOD', 0,
                    buffer_gets_delta)) over () > 0
            then round(100 * sum(decode(command_name, 'PL/SQL EXECUTE', 0, 'CALL METHOD', 0,
                                        buffer_gets_delta))
                                    over (order by buffer_gets_delta desc) 
                    / sum(decode(command_name, 'PL/SQL EXECUTE', 0, 'CALL METHOD', 0,
                            buffer_gets_delta)) over (), 1)
            end as tot_pct_gets,
        dbid,
